...
void parseConfigFile()
{
	string line;
	char line_str[1024];
	ifstream cfgfile("interface_VM1.cfg");
...
*/
    	...
    	// Open the file in read mode and store its pointer in fp
    	fp=fopen("TripFromPRCB.csv","r");
    	// Check if the file was opened successfully
    	if(fp==NULL) {
	        printf("File cannot open! " );
	        exit(0);
     	}
	    // Read values from the file and store them in the CBstval array
		for (i=0; i<4; i++) {
			fscanf(fp,"%d,", &CBstval[i]);
		}
    	// Close the file
    	fclose(fp);
	    // Assign the values read from the file to the digitalOut array in stations_data[0]
		for (int i = 0; i <4; i++ )

			stations_data[0].digitalOut[i] = CBstval[i] ;
			
			// Print the values of the digitalOut array in stations_data[0]
			printf(" CB-33KV: %d\n CB-TRSF: %d\n CB-11KV: %d\n CB-Load: %d\n \n", stations_data[0].digitalOut[0], stations_data[0].digitalOut[1], stations_data[0].digitalOut[2], stations_data[0].digitalOut[3]);
/* 
READ DATA FROM CSV [end]
*/
/*
