...
void parseConfigFile()
{
	string line;
	char line_str[1024];
	ifstream cfgfile("interface_VM2.cfg");
...
/* WRITE DATA TO CSV [begin] */
    	...
    	// Set the elements of the CBstval array to values from stations_data
    	CBstval [0] = stations_data[0].digitalOut[0]; //open_CB-33KV
    	CBstval [1] = stations_data[0].digitalOut[1]; //open_CB-TRSF
    	CBstval [2] = stations_data[0].digitalOut[2]; //open_CB-11KV
    	CBstval [3] = stations_data[0].digitalOut[3]; //open_CB-FDR

    	// Open the file in write mode and store its pointer in fp
    	fp=fopen("TripFromPLC.csv","w");
